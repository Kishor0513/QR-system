...
// Generate QR code for a single product (PNG only)
async function generateQRCode(product) {
  ...
  try {
    ...
    console.error(
      `✗ Failed to generate QR for ${product.name}:`,
      error.message
    );
    return false;
  }
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    process.stdout.write(
      `\rGenerating QR codes... ${i + 1}/${products.length}`
    );
  ...
  console.log(`\n\n✅ Successfully generated ${successCount} PNG QR codes`);
  if (failCount > 0) {
    console.log(`⚠️  Failed to generate ${failCount} QR codes`);
  }
  console.log(`📁 QR codes (PNG) saved to: ${OUTPUT_DIR}`);
