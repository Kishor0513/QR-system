...
function generateProductId(productName, category, productLabel) {
    ...
    const label = cleanString(productLabel).toLowerCase().replace(/[^a-z0-9]+/g, '-');
        ...
        fs.createReadStream(csvPath)
            ...
            .on('data', (row) => {
                if (isValidProduct(row)) {
                    ...
                    const productLabel = cleanString(row['Product label\n(3-5 Characters)']);
                    ...
                    const product = {
                        ...
                        websiteLink: cleanString(row['Product Image\nWebsite Link']) || cleanString(row['Product Image Website Link']),
            ...
            .on('end', () => {
                console.log(`✓ Processed ${csvPath}: ${products.length} valid products`);
                resolve(products);
            })
            .on('error', reject);
    });
